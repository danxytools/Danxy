...
│ [   ${RED}17${GREEN}  ]  │ ${YELLOW}TRACK REKENING${GREEN}          │          │
...
╰────────────┴─────────────────────────┴──────────╯
│             ${BG_RED}${YELLOW}YT${NC}${BG_RED}: ${BLUE}${BG_RED}DanxyBot ${YELLOW}${BG_RED}TT${NC}${BG_RED}: ${BLUE}${BG_RED}Qwela.38${NC}${GREEN}           │
╰─────────────────────────────────────────────────╯${NC}" | lolcat
  #echo -e "${CYAN}"
  #echo "     ╭─══════════════════════════════════─╮"
...
osint_rekening() {
    echo "Masukkan Nama Bank (bca/mandiri/bri/bni/dll): "
    read bank
    if [[ -z "$bank" ]]; then
    ...
    echo "[🏦] Mencari informasi rekening..."
    response=$(curl -s "https://api-rekening.lfourr.com/getAccount?bank=$bank&norek=$rekening")

    if [[ "$response" == "null" || -z "$response" ]]; then
        echo "[!] Data rekening tidak ditemukan"
        return
    fi

    nama=$(echo "$response" | jq -r '.data.accountName')
    bankName=$(echo "$response" | jq -r '.data.bankName')
    ...
    if [[ "$nama" != "null" ]]; then
        ...
        echo "[!] Rekening tidak terdaftar atau salah input"
    fi
}
...
osint_nik() {
    ...
    if [[ ${#nik} -ne 16 ]]; then
        echo "[❌] NIK tidak valid, harus 16 digit!"
        return
    fi

    if ! command -v jq > /dev/null; then
        echo "[❌] 'jq' belum terinstal. Instal dulu dengan: pkg install jq"
        return
    fi

    echo "[🔍] Melacak informasi dari NIK..."
    curl -s "https://api-rekayasa.com/nik?nik=$nik" | jq
}
